...
-- Server
function GrowingCottonPlant.server_onCreate( self )
...
function GrowingCottonPlant.server_onReceiveUpdate( self )
	self:sv_performUpdate()
end

function GrowingCottonPlant.sv_performUpdate( self )
...
function GrowingCottonPlant.sv_updateTicks( self, ticks )
	if not self.sv.regrown and sm.exists( self.harvestable ) then
		...
		if growFraction >= 1.0 then
			sm.harvestable.createHarvestable( hvs_farmables_cottonplant, self.harvestable.worldPosition, self.harvestable.worldRotation )
